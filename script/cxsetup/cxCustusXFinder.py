...
class CustusXFinder:
    ...
    def syncTagOrBranchToParent(self, cx_path):
        ...
        if tag:
            print 'Checking out CustusX to same tag=%s as NorMIT' % tag
            ...
            if branch=="HEAD": # jenkins always checkout a SHA, thus the repo always start out detached
                print "NorMIT repo is detached, guess branch=master as we have no more info"
                branch = 'master'

            print 'Checking out/pulling CustusX to same branch=%s as NorMIT' % branch
    ...
    def _parseArgs(self):
        ...
        parser.add_argument('--main_branch', default='develop', dest='main_branch')
